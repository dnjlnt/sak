...
class D0142 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0142_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
            ...
            $kebersihan_badan = $this->input->post("kebersihan_badan");
            $nafsu_makan = $this->input->post("nafsu_makan");
            $demam = $this->input->post("demam");
            $kemerahan = $this->input->post("kemerahan");
            $nyeri = $this->input->post("nyeri");
            $bengkak = $this->input->post("bengkak");
            $vesikel = $this->input->post("vesikel");
            $cairan_berbau_busuk = $this->input->post("cairan_berbau_busuk");
            $spuntum = $this->input->post("spuntum");
            $drainase_purulen = $this->input->post("drainase_purulen");
            $piuria = $this->input->post("piuria");
            $periode_malaise = $this->input->post("periode_malaise");
            $periode_menggigil = $this->input->post("periode_menggigil");
            $letargi = $this->input->post("letargi");
            $gangguan_kognitif = $this->input->post("gangguan_kognitif");
            $sel_darah_putih = $this->input->post("sel_darah_putih");
            $kultur_darah = $this->input->post("kultur_darah");
            $kultur_urine = $this->input->post("kultur_urine");
            $kultur_sputum = $this->input->post("kultur_sputum");
            $kultur_area_luka = $this->input->post("kultur_area_luka");
            $kultur_feses = $this->input->post("kultur_feses");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0142($newDiagRecordNumber, $durasi_intervensi, $kebersihan_tangan, $kebersihan_badan, $nafsu_makan, $demam, $kemerahan, $nyeri, $bengkak, $vesikel, $cairan_berbau_busuk, $spuntum, $drainase_purulen, $piuria, $periode_malaise, $periode_menggigil, $letargi, $gangguan_kognitif, $sel_darah_putih, $kultur_darah, $kultur_urine, $kultur_sputum, $kultur_area_luka, $kultur_feses, $created_by, $created_dttm);
