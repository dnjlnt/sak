...
class D0063 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0063_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
			...
			$mempertahankan_makanan = $this->input->post("mempertahankan_makanan");
            $reflek_menelan = $this->input->post("reflek_menelan");
            $mengosongkan_mulut = $this->input->post("mengosongkan_mulut");
            $kemampuan_mengunyah = $this->input->post("kemampuan_mengunyah");
            $usaha_menelan = $this->input->post("usaha_menelan");
            $pembentukan_bolus = $this->input->post("pembentukan_bolus");
            $frekuensi_tersedak = $this->input->post("frekuensi_tersedak");
            $batuk = $this->input->post("batuk");
            $muntah = $this->input->post("muntah");
            $refluks_lambung = $this->input->post("refluks_lambung");
            $gelisah = $this->input->post("gelisah");
            $regurgitasi = $this->input->post("regurgitasi");
            $produksi_saliva = $this->input->post("produksi_saliva");
            $penerimaan_makanan = $this->input->post("penerimaan_makanan");
            $kualitas_suara = $this->input->post("kualitas_suara");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0063($newDiagRecordNumber, $durasi_intervensi, $mempertahankan_makanan, $reflek_menelan, $mengosongkan_mulut, $kemampuan_mengunyah, $usaha_menelan, $pembentukan_bolus, $frekuensi_tersedak, $batuk, $muntah, $refluks_lambung, $gelisah, $regurgitasi, $produksi_saliva, $penerimaan_makanan, $kualitas_suara, $created_by, $created_dttm);
