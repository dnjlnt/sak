...
class D0032 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0032_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
			...
			$porsi_makan = $this->input->post("porsi_makan");
            $otot_pengunyah = $this->input->post("otot_pengunyah");
            $otot_menelan = $this->input->post("otot_menelan");
            $serum_albumin = $this->input->post("serum_albumin");
            $meningkatkan_nutrisi = $this->input->post("meningkatkan_nutrisi");
            $pengetahuan_makanan_sehat = $this->input->post("pengetahuan_makanan_sehat");
            $pengetahuan_minuman_sehat = $this->input->post("pengetahuan_minuman_sehat");
            $pengetahuan_standar_nutrisi = $this->input->post("pengetahuan_standar_nutrisi");
            $penyiapan_makanan_aman = $this->input->post("penyiapan_makanan_aman");
            $penyiapan_minuman_aman = $this->input->post("penyiapan_minuman_aman");
            $sikap_tujuan_kesehatan = $this->input->post("sikap_tujuan_kesehatan");
            $cepat_kenyang = $this->input->post("cepat_kenyang");
            $nyeri_abdomen = $this->input->post("nyeri_abdomen");
            $sariawan = $this->input->post("sariawan");
            $rambut_rontok = $this->input->post("rambut_rontok");
            $diare = $this->input->post("diare");
            $berat_badan = $this->input->post("berat_badan");
            $index_massa_tubuh = $this->input->post("index_massa_tubuh");
            $frekuensi_makan = $this->input->post("frekuensi_makan");
            $bising_usus = $this->input->post("bising_usus");
            $tebal_lipatan_kulit = $this->input->post("tebal_lipatan_kulit");
            $membran_mukosa = $this->input->post("membran_mukosa");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0032($newDiagRecordNumber, $durasi_intervensi, $porsi_makan, $otot_pengunyah, $otot_menelan, $serum_albumin, $meningkatkan_nutrisi, $pengetahuan_makanan_sehat, $pengetahuan_minuman_sehat, $pengetahuan_standar_nutrisi, $penyiapan_makanan_aman, $penyiapan_minuman_aman, $sikap_tujuan_kesehatan, $cepat_kenyang, $nyeri_abdomen, $sariawan, $rambut_rontok, $diare, $berat_badan, $index_massa_tubuh, $frekuensi_makan, $bising_usus, $tebal_lipatan_kulit, $membran_mukosa, $created_by, $created_dttm);
