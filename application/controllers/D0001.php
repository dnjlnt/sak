...
class D0001 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0001_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
			...
			$sinkron_bantuan_ventilator = $this->input->post("sinkron_bantuan_ventilator");
            $otot_bantu_nafas = $this->input->post("otot_bantu_nafas");
            $gasping = $this->input->post("gasping");
            $nafas_dangkal = $this->input->post("nafas_dangkal");
            $agitasi = $this->input->post("agitasi");
            $lelah = $this->input->post("lelah");
            $perasaan_kuatir = $this->input->post("perasaan_kuatir");
            $fokus_pernafasan = $this->input->post("fokus_pernafasan");
            $nafas_paradoks_abdominal = $this->input->post("nafas_paradoks_abdominal");
            $diaforesis = $this->input->post("diaforesis");
            $frekuensi_nafas = $this->input->post("frekuensi_nafas");
            $nilai_gas_darah = $this->input->post("nilai_gas_darah");
            $upaya_napas = $this->input->post("upaya_napas");
            $auskultasi_suara = $this->input->post("auskultasi_suara");
            $warna_kulit = $this->input->post("warna_kulit");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0001($newDiagRecordNumber, $durasi_intervensi, $sinkron_bantuan_ventilator, $otot_bantu_nafas, $gasping, $nafas_dangkal, $agitasi, $lelah, $perasaan_kuatir, $fokus_pernafasan, $nafas_paradoks_abdominal, $diaforesis, $frekuensi_nafas, $nilai_gas_darah, $upaya_napas, $auskultasi_suara, $warna_kulit, $created_by, $created_dttm);
			
			if ($saveLuaranKeperawatan == true) {
				echo "success-save-all-record";
				$this->session->set_userdata('patient_mr', $patient_mr);
				$this->session->userdata('patient_mr');
