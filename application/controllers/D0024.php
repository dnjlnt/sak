...
class D0024 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0024_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
			...
			$elastisitas = $this->input->post("elastisitas");
            $hidrasi = $this->input->post("hidrasi");
            $perfusi_jaringan = $this->input->post("perfusi_jaringan");
            $kerusakan_jaringan = $this->input->post("kerusakan_jaringan");
            $kerusakan_lapisan_kulit = $this->input->post("kerusakan_lapisan_kulit");
            $nyeri = $this->input->post("nyeri");
            $pendarahan = $this->input->post("pendarahan");
            $kemerahan = $this->input->post("kemerahan");
            $hematoma = $this->input->post("hematoma");
            $pigmentasi_abnormal = $this->input->post("pigmentasi_abnormal");
            $jaringan_parut = $this->input->post("jaringan_parut");
            $nekrosis = $this->input->post("nekrosis");
            $abrasi_kornea = $this->input->post("abrasi_kornea");
            $suhu_kulit = $this->input->post("suhu_kulit");
            $sensasi = $this->input->post("sensasi");
            $tekstur = $this->input->post("tekstur");
            $pertumbuhan_rambut = $this->input->post("pertumbuhan_rambut");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0024($newDiagRecordNumber, $durasi_intervensi, $elastisitas, $hidrasi, $perfusi_jaringan, $kerusakan_jaringan, $kerusakan_lapisan_kulit, $nyeri, $pendarahan, $kemerahan, $hematoma, $pigmentasi_abnormal, $jaringan_parut, $nekrosis, $abrasi_kornea, $suhu_kulit, $sensasi, $tekstur, $pertumbuhan_rambut, $created_by, $created_dttm);
