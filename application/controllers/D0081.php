...
class D0081 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0081_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
			...
			$menerima_kehilangan = $this->input->post("menerima_kehilangan");
            $kehilangan = $this->input->post("kehilangan");
            $perasaan_berguna = $this->input->post("perasaan_berguna");
            $perasaan_sedih = $this->input->post("perasaan_sedih");
            $perasaan_bersalah = $this->input->post("perasaan_bersalah");
            $menangis = $this->input->post("menangis");
            $mimpi_buruk = $this->input->post("mimpi_buruk");
            $fobia = $this->input->post("fobia");
            $marah = $this->input->post("marah");
            $panik = $this->input->post("panik");
            $pola_tidur = $this->input->post("pola_tidur");
            $konsentrasi = $this->input->post("konsentrasi");
            $imunitas = $this->input->post("imunitas");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0081($newDiagRecordNumber, $durasi_intervensi, $menerima_kehilangan, $kehilangan, $perasaan_berguna, $perasaan_sedih, $perasaan_bersalah, $menangis, $mimpi_buruk, $fobia, $marah, $panik, $pola_tidur, $konsentrasi, $imunitas, $created_by, $created_dttm);
