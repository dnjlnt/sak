...
class D0085 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0085_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
			...
			$mendengar_bisikan = $this->input->post("mendengar_bisikan");
            $melihat_bayangan = $this->input->post("melihat_bayangan");
            $indra_penciuman = $this->input->post("indra_penciuman");
            $indra_peraba = $this->input->post("indra_peraba");
            $pengecapan = $this->input->post("pengecapan");
            $dostorsi_sensori = $this->input->post("dostorsi_sensori");
            $perilaku_halusinasi = $this->input->post("perilaku_halusinasi");
            $menarik_diri = $this->input->post("menarik_diri");
            $melamun = $this->input->post("melamun");
            $curiga = $this->input->post("curiga");
            $mondarmandir = $this->input->post("mondarmandir");
            $respon_stimulus = $this->input->post("respon_stimulus");
            $konsentrasi = $this->input->post("konsentrasi");
            $orientasi = $this->input->post("orientasi");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0085($newDiagRecordNumber, $durasi_intervensi, $mendengar_bisikan, $melihat_bayangan, $indra_penciuman, $indra_peraba, $pengecapan, $dostorsi_sensori, $perilaku_halusinasi, $menarik_diri, $melamun, $curiga, $mondarmandir, $respon_stimulus, $konsentrasi, $orientasi, $created_by, $created_dttm);
