...
class D0005 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0005_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
			...
			$ventilasi_semenit = $this->input->post("ventilasi_semenit");
            $kapasitas_vital = $this->input->post("kapasitas_vital");
            $diameter_throraks = $this->input->post("diameter_throraks");
            $anterior = $this->input->post("anterior");
            $tekanan_ekspirasi = $this->input->post("tekanan_ekspirasi");
            $tekanan_inspirasi = $this->input->post("tekanan_inspirasi");
            $dispnea = $this->input->post("dispnea");
            $otot_bantu_napas = $this->input->post("otot_bantu_napas");
            $fase_ekspirasi = $this->input->post("fase_ekspirasi");
            $ortopnea = $this->input->post("ortopnea");
            $pernapasan_pursed = $this->input->post("pernapasan_pursed");
            $frekuensi_nafas = $this->input->post("frekuensi_nafas");
            $kedalaman_nafas = $this->input->post("kedalaman_nafas");
            $ekskursi_dada = $this->input->post("ekskursi_dada");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0005($newDiagRecordNumber, $durasi_intervensi, $sinkron_bantuan_ventilator, $otot_bantu_nafas, $gasping, $nafas_dangkal, $agitasi, $lelah, $perasaan_kuatir, $fokus_pernafasan, $nafas_paradoks_abdominal, $diaforesis, $frekuensi_nafas, $nilai_gas_darah, $upaya_napas, $auskultasi_suara, $warna_kulit, $created_by, $created_dttm);
