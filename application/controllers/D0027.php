...
class D0027 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0027_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
            ...
            $mengantuk = $this->input->post("mengantuk");
            $pusing = $this->input->post("pusing");
            $lelah = $this->input->post("lelah");
            $keluhan_lapar = $this->input->post("keluhan_lapar");
            $gemetar = $this->input->post("gemetar");
            $berkeringat = $this->input->post("berkeringat");
            $mulut_kering = $this->input->post("mulut_kering");
            $rasa_haus = $this->input->post("rasa_haus");
            $perilaku_aneh = $this->input->post("perilaku_aneh");
            $kesulitan_bicara = $this->input->post("kesulitan_bicara");
            $kadar_glukosa_darah = $this->input->post("kadar_glukosa_darah");
            $kadar_glukosa_urine = $this->input->post("kadar_glukosa_urine");
            $palpitasi = $this->input->post("palpitasi");
            $perilaku = $this->input->post("perilaku");
            $jumlah_urine = $this->input->post("jumlah_urine");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0027($newDiagRecordNumber, $durasi_intervensi, $koordinasi, $kesadaran, $mengantuk, $pusing, $lelah, $keluhan_lapar, $gemetar, $berkeringat, $mulut_kering, $rasa_haus, $perilaku_aneh, $kesulitan_bicara, $kadar_glukosa_darah, $kadar_glukosa_urine, $palpitasi, $perilaku, $jumlah_urine, $created_by, $created_dttm);
