...
class D0029 extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('diagnosa/form/d0029_form.php', $data);
	}
	...
	public function saveNewDiagnosa() {
		...
		if ($saveMasterDiagRecord == true) {
			...
			$perlekatan_bayi = $this->input->post("perlekatan_bayi");
            $kemampuan_ibu = $this->input->post("kemampuan_ibu");
            $miksi_bayi = $this->input->post("miksi_bayi");
            $berat_badan_bayi = $this->input->post("berat_badan_bayi");
            $tetesan_asi = $this->input->post("tetesan_asi");
            $suplai_asi = $this->input->post("suplai_asi");
            $putting_tidak_lecet = $this->input->post("putting_tidak_lecet");
            $kepercayaan_diri_ibu = $this->input->post("kepercayaan_diri_ibu");
            $bayi_tidur = $this->input->post("bayi_tidur");
            $payudara_kosong = $this->input->post("payudara_kosong");
            $intake_bayi = $this->input->post("intake_bayi");
            $hisapan_bayi = $this->input->post("hisapan_bayi");
            $lecet_putting = $this->input->post("lecet_putting");
            $kelelahan_maternal = $this->input->post("kelelahan_maternal");
            $kecemasan_maternal = $this->input->post("kecemasan_maternal");
            $bayi_rewel = $this->input->post("bayi_rewel");
            $bayi_menangis = $this->input->post("bayi_menangis");

            $saveLuaranKeperawatan = $this->ModelDiagnosa->saveLuaranKeperawatand0029($newDiagRecordNumber, $durasi_intervensi, $perlekatan_bayi, $kemampuan_ibu, $miksi_bayi, $berat_badan_bayi, $tetesan_asi, $suplai_asi, $putting_tidak_lecet, $kepercayaan_diri_ibu, $bayi_tidur, $payudara_kosong, $intake_bayi, $hisapan_bayi, $lecet_putting, $kelelahan_maternal, $kecemasan_maternal, $bayi_rewel, $bayi_menangis, $created_by, $created_dttm);
